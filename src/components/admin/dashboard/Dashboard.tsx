...
export function Dashboard({ principal, denied }: { principal: PrincipalView; denied: boolean }) {
  const { data, error, loading, reload } = useAdminData<DashboardSnapshot>("/api/admin/dashboard");
  const has = (permission: Permission) => principal.permissions.includes(permission);

  const shortcuts = navFor(principal.permissions)
  ...
  return (
    <>
      <PageHeader
        title="ড্যাশবোর্ড"
        description={`আসসালামু আলাইকুম, ${principal.name} (${principal.categoryName})। আপনার কাজ ও দায়িত্বের সারসংক্ষেপ।`}
        actions={
          <Button
            size="sm"
            onClick={reload}
      ...
      {denied ? (
        <div className="mb-5">
          <Notice tone="warn">যে পেজটি খুলতে চেয়েছিলেন, সেটি দেখার অনুমতি আপনার নেই।</Notice>
        </div>
      ) : null}
              ...
              {[
                {
                  ...
                  label: "রিভিউয়ের অপেক্ষায় উত্তর",
                  value: data.queues.openReviews,
                  hint: "ব্যবহারকারী ভুল বা অসম্পূর্ণ বলেছেন",
                },
                {
                  href: "/admin/help",
                  permission: "help.view" as const,
                  label: "আলেমের কাছে খোলা প্রশ্ন",
                  value: data.queues.openHelp,
                  hint: "উত্তরের অপেক্ষায়",
                },
                {
                  href: "/admin/masail",
                  permission: "masail.write" as const,
                  label: "প্রকাশিত মাসআলা",
                  value: data.queues.publishedMasail,
                  hint: "আলেমদের লেখা, পাবলিক পেজে",
                },
              ]
                ...
                .map((item) => (
                  ...
                  >
                    <Stat
                      ...
                      hint={item.hint}
                      tone={item.href !== "/admin/masail" && item.value > 0 ? "warn" : undefined}
                    />
                  </Link>
                ))}
            </div>
          ...
          {has("monitor.view") ? (
            <div className="grid gap-5 lg:grid-cols-2">
              <Card
                ...
                actions={
                  has("corpus.manage") ? <CardLink href="/admin/corpus">পরিচালনা</CardLink> : null
                }
              >
                {data.corpus ? (
              ...
              <Card
                ...
                actions={
                  has("database.manage") ? (
                    <CardLink href="/admin/database">ডাটাবেস</CardLink>
                  ) : null
                }
              >
                {data.storage ? (
                      ...
                      {data.storage.collections.map((row) => (
                        ...
                        >
                          {has("database.manage") ? (
                            ...
                            <span className="min-w-0 truncate font-mono text-(--text-1)">
                              {row.name}
                            </span>
                          )}
              ...
              <Card
                ...
                actions={has("files.manage") ? <CardLink href="/admin/files">ফাইল</CardLink> : null}
              >
                <dl className="grid grid-cols-2 gap-x-4 gap-y-3 text-sm">
                  <div>
